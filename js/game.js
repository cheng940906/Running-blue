...
person.prototype={
    draw:function(){
        ...
        this.cobj.drawImage(this[this.status][this.state],0,0,100,100,0,0,this.width,this.height);
        this.cobj.restore();
    }
};
...
game.prototype={
        ...
        for(var i=0;i<that.hinderArr.length;i++){
            ...
            if(hitPix(that.canvas,that.cobj,that.person,that.hinderArr[i])){
                if(!that.hinderArr[i].flag){
                    that.person.life--;
                    that.dieA.play();
                    that.progress.style.background="orangered";
    ...
    zhezhao:function(){
        ...
        mask.click(function(){
            ...
            that.zobj.y=that.person.y+that.person.height/2-25;
